		...
		<View>
			<View key={`View-${sectionData[section].id}`}>
				<Section
					backClick={backClick}
					press={clickHandler}
					key={`Section-${sectionData[section].id}`}
					carouselData={sectionData[section].carouselle}
					headerImg={sectionData[section].img}
					mainHeader={sectionData[section].title}
				/>
			</View>
		</View>
	);
				...
				return (
					<View key={`ViewTwo-${meeting.id}`}>
						<Meeting
							...
							carouselData={meeting.carouselle}
							headerImg={meeting.img}
							mainHeader={meeting.title}
						/>
					</View>
				);
			})}
